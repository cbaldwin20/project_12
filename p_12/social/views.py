...
def index(request, need="All Needs"):
    ...
    # if the user has skills, then will filter all of the skills to find matching
    # projects and positions.
        ...
        query = functools.reduce(operator.or_, (
            Q(
              position_name__icontains=the_skill)|Q(
position_description__icontains=the_skill) for the_skill in my_skills_name_list))
    ...
    if position_pk:
        ...
        if action == "apply":
            ...
            messages.success(request,
                        'You applied for {}'.format(the_position.position_name))
            ...
            if projects_formset.is_valid():
                if skills_formset.is_valid():
                    ...
                    final_user_profile.url_slug = "{}-{}".format(seconds,
                                               slugify(final_user_profile.name))
...
def profile(request, url_slug=None):
    ...
    positions = models.Position.objects.filter(position_filled_user=profile.user)
    return render(request,
                  'profile.html',
                  {'profile': profile,
                  'positions': positions,
